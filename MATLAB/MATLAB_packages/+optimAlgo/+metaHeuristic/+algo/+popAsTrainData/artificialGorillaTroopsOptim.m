...
function [optimdInst,bestUnit,bestCostProg] = artificialGorillaTroopsOptim(trainIter,costFunc,popInst)
    ...
    % Pre-allocate for optimized population
    % optimdInst = optimAlgo.popClass(popSize,itemSize,itemDim,dataLim,unitOrient);
    
    % Position of Silver Back
    % silverBackPos = zeros(unitRowDim,unitColDim);
    ...
    for i = 1:trainIter
        ...
        for unitIndex = 1:popSize
            ...
            elseif randnum >= 0.5
                Z = (C - (-C))*rand(1)+(-C);    % eq 6
                H = Z.*popData(:,:,unitIndex);	% eq 7
                r2 = (1 - 0)*rand(1)+0;
                GX.popData(:,:,unitIndex) = (r2 - C).*...
                    popData(:,:,unitIndex) + L.*H;    % eq 8
                
            else
                randIndex = randi(popSize);
                r3 = (1 - 0)*rand(1)+0;
                GX.popData(:,:,unitIndex) = popData(:,:,unitIndex) - L.*(L.*...
                    (popData(:,:,unitIndex) - popData(:,:,randIndex)) +...
                    r3.*(popData(:,:,unitIndex) - popData(:,:,randIndex)));
            end
        end
        
        % Evaluate cost of all units in GX
        GX.popCost = costFunc(GX);
        
        % Find unit indices with better cost in GX than popData
        betterUnitIndex = find(GX.popCost < popCost);
        
        % The replace better unit indices from popData with GX
        popData(:,:,betterUnitIndex) = GX.popData(:,:,betterUnitIndex);
        popCost(betterUnitIndex,1) = GX.popCost(betterUnitIndex,1);
        
        % Find the best cost assign it as the Silver Back
        [silverBackCost, silverBackIndex] = min(popCost);
        silverBackPos = popData(:,:,silverBackIndex);
        ...
        for unitIndex = 1:popSize
            if C >= W
                ...
                for j = 1:popSize
                    sum = sum + popData(:,:,randIndex(j));
                end
                M = (abs( (1/popSize).*sum  ).^g).^(1/g);
                GX.popData(:,:,unitIndex) = L.*M.*(popData(:,:,unitIndex)...
                    - silverBackPos) + popData(:,:,unitIndex);
            else
                ...
                GX.popData(:,:,unitIndex) = silverBackPos - ...
                    (silverBackPos.*Q-popData(:,:,unitIndex).*Q).*A;
            end
        end
